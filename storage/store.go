...
type Storer interface {
	// ReadAt defines behavior for a random access reader that reads bytes from the underlying storage on page starting at offset.
	...
	ReadAt(page int, offset int64) (*pb.Entry, error)
	// Scan reads the entry at page and offset and returns the entry, the next page, and the next offset. It returns an error if it can't
	// read the entry or there's a problem with the underlying storage. An io.EOF is returned when there is nothing left to scan.
	Scan(page int, offset int64) (*pb.Entry, int, int64, error)
	// Append writes data to the tail of the underlying storage, returning the page and offset where the data starts.
